...
**Phase**: session work chunk (matt-grilling, implementation, QA). Ends on completion.
...
| **`/matt-handoff`** | Write portable markdown file. Seed new session. |
...
Next phase needs **primary source** or ~150k tokens remaining (smart zone) => yes. Example: matt-grilling -> implementation. Implementation needs verbatim reasoning. Continue costs zero + loses zero. Rule out first.
...
| Secondary (`/compact`, `/matt-handoff`) | Lossy | Low | High |
