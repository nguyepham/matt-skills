...
- **Map**: single issue. labels `matt-wayfinder:map`. holds notes, decisions, fog. `glab issue create --label matt-wayfinder:map`. (native epics work on supported tiers).
- **Child ticket**: prepend `Part of #<map>` in description. labels `matt-wayfinder:<type>` (`research`, `matt-prototype`, `matt-grilling`, `task`). assignment => driving dev.
