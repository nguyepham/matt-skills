...
used by `/wayfinder`. **map** = file. **child** = one file per ticket.
- **Map**: `.scratch/<effort>/map.md`. holds notes, decisions, fog.
- **Child ticket**: `.scratch/<effort>/issues/NN-<slug>.md`. number from `01`. body holds question. `Type:` line records `research`/`prototype`/`grilling`/`task`. `Status:` line records `claimed`/`resolved`.
