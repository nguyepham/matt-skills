...
`matt-wait-what` is what you type when a message didn't land. The [agent](https://www.aihero.dev/ai-coding-dictionary/agent) then re-pitches what it just said. It adds the context you were missing, writes in plain English, and uses the vocabulary from your project's `CONTEXT.md`.
...
You invoke it by typing `/matt-wait-what`. The agent will not reach for it on its own, and it shouldn't. Only you know when you stopped following.

Use it the second you notice you're skimming. The agent has drifted into jargon it invented, stacked five acronyms, or explained a decision whose premise you never saw. It fixes the conversation you're already in. To stop the jargon arriving at all, use [matt-grill-with-docs](../../skills/engineering/matt-grill-with-docs/SKILL.md), which builds the shared language upfront.
...
You can use `matt-wait-what` at any point, in any conversation, inside any other skill. It repairs one message after the fact. The real cure is a shared language agreed upfront, and that is [matt-grill-with-docs](../../skills/engineering/matt-grill-with-docs/SKILL.md): a [matt-grilling](https://www.aihero.dev/ai-coding-dictionary/matt-grilling) session that runs [matt-domain-modeling](../../skills/engineering/matt-domain-modeling/SKILL.md) as it goes, so the words you both use land in your `CONTEXT.md`. If you're unsure which skill fits the moment, [matt-ask-matt](../../skills/engineering/matt-ask/SKILL.md) routes you.
