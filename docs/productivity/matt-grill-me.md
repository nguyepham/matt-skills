...
You invoke this by typing `/matt-grill-me`; the [agent](https://www.aihero.dev/ai-coding-dictionary/agent) won't reach for it on its own. Start it in a **fresh conversation**, not on top of a plan you already had an agent write.
...
Which of the three matt-grilling skills you want depends on what is in front of you:

- **Anything, anywhere**: `matt-grill-me`. It needs no repo and writes no files, and the subject doesn't have to be code.
- **A codebase to align against**: [matt-grill-with-docs](../engineering/matt-grill-with-docs.md). The same interview, but [stateful](https://www.aihero.dev/ai-coding-dictionary/stateful): it reads your code and keeps what it learns in `CONTEXT.md` and ADRs.
- **Too big for one session**: [matt-wayfinder](../engineering/matt-wayfinder.md). It charts the effort as a map and runs matt-grilling sessions inside it.
...
Some questions can be answered by talking. Others can't, and no amount of matt-grilling will get you there.

"One long form or three pages?" and "how should this interaction feel?" are **ungrillable**: they need something to react to. When you hit one, stop matt-grilling. Build the throwaway version with [matt-prototype](../engineering/matt-prototype.md), look at it, then come back and answer in one line.
...
Say so. "I don't know" is a real answer, and a question you can't answer is usually a sign to matt-prototype rather than to guess.

**Do I start a fresh session before writing the spec?**
No. The value of the session is the [context](https://www.aihero.dev/ai-coding-dictionary/context) you just built. Hand the same conversation straight to [matt-to-spec](../engineering/matt-to-spec.md).
