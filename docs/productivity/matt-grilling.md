...
`matt-grilling` is the interview loop that stress-tests a plan, a decision, or an idea before anyone acts on it. It maps the subject as a **design tree**: every decision branches into the decisions that hang off it, and interviews you branch by branch until nothing is left silently assumed.
...
Type `/matt-grilling`, or the [agent](https://www.aihero.dev/ai-coding-dictionary/agent) reaches for it on its own when a task fits. It is the only [skill](https://www.aihero.dev/ai-coding-dictionary/skill) in the matt-grilling family that is model-invoked, which is why you rarely type it: usually a skill you *did* type is running it for you.

Typing `/matt-grilling` directly gets you the plain interview and nothing else. Where you want something more than that:

| What you have | Reach for |
| --- | --- |
| You aren't working in a working directory | [matt-grill-me](matt-grill-me.md): the same [session](https://www.aihero.dev/ai-coding-dictionary/session), under a name the agent will never fire by itself |
| You are in a working directory | [matt-grill-with-docs](../engineering/matt-grill-with-docs.md): the same session, and it writes `CONTEXT.md` and ADRs as it goes |
| An effort too big to hold in one session | [matt-wayfinder](../engineering/matt-wayfinder.md): it charts a map and runs matt-grilling inside the decision tickets |
| A question that talking cannot settle: how something should look or feel | [matt-prototype](../engineering/matt-prototype.md): build the throwaway version, then come back |
| A skill of your own that needs an interview | Invoke `/matt-grilling` from it, rather than writing another interview |
...
| How long a session should run, what to do with a question you can't answer by talking, how to avoid nodding along | [matt-grill-me](matt-grill-me.md) |
| What gets written to `CONTEXT.md`, what becomes an ADR | [matt-grill-with-docs](../engineering/matt-grill-with-docs.md) |
...
```
When matt-grilling, ask one question at a time.
```
...
**I installed `matt-grill-me` on its own and nothing happens.**
`matt-grill-me` is a one-line skill whose whole body is "run a `/matt-grilling` session", so it needs this skill installed too. The same is true of `matt-grill-with-docs`, which additionally needs [matt-domain-modeling](../engineering/matt-domain-modeling.md). Installing the whole set avoids the problem; installing selectively means installing the primitives as well.

**`matt-grill-with-docs` ran, but it never loaded `matt-grilling`.**
A real and unfixed rough edge, reported across [harnesses](https://www.aihero.dev/ai-coding-dictionary/harness) and models: a skill that names another skill does not reliably cause that skill to load, and `matt-grill-with-docs` names two. The tell is a session that asks everything at once with no recommendations attached: that is the model improvising an interview rather than running this one. Asking the agent directly whether it loaded `matt-grilling` and `matt-domain-modeling` usually recovers it.
...
`matt-grilling` is a **primitive**, not a step you schedule: the single source of truth for the interview technique, kept in one place so every skill that needs an interview reaches for it instead of inventing one. [matt-grill-me](matt-grill-me.md) and [matt-grill-with-docs](../engineering/matt-grill-with-docs.md) are its two user-invoked front doors, and `matt-grill-with-docs` is where the main build chain begins, ahead of [matt-to-spec](../engineering/matt-to-spec.md). [matt-wayfinder](../engineering/matt-wayfinder.md) runs it to resolve decision tickets, [triage](../engineering/matt-triage.md) to grill a vague report into a workable one, and [matt-improve-codebase-architecture](../engineering/matt-improve-codebase-architecture.md) to walk the tree once you have picked a candidate to deepen. When you are unsure which entry point fits, [matt-ask-matt](../engineering/matt-ask-matt.md) routes you.
