...
`matt-to-questionnaire` turns a decision you can't settle on your own into a **questionnaire**: a Markdown document you hand to the one person who holds what you're missing, for them to fill in async or for the two of you to work through in a meeting.
...
You invoke this by typing `/matt-to-questionnaire`; the [agent](https://www.aihero.dev/ai-coding-dictionary/agent) won't reach for it on its own.
...
| --- | --- |
| Your own head, unsharpened | [matt-grill-me](../../skills/productivity/matt-grill-me/SKILL.md) |
| The codebase | [matt-grill-with-docs](../../skills/engineering/matt-grill-with-docs/SKILL.md) |
| Someone else's head | `matt-to-questionnaire` |
| Nobody's head yet, the question needs something to react to | [matt-prototype](../../skills/engineering/matt-prototype/SKILL.md) |

The common case is a [matt-grilling](https://www.aihero.dev/ai-coding-dictionary/matt-grilling) session that stalls: some of what surfaced isn't yours to answer. Run `/matt-to-questionnaire` in that same conversation to take those questions offline, then bring the answers back and carry on.
...
**Isn't this just `/matt-grill-me` in batch mode?**
No, and the distinction is worth holding. `matt-grill-me` already asks in **rounds**: the whole frontier at once, then recomputed from your answers, so the "give me all the questions at once" need is met there. `matt-to-questionnaire` is about a different axis: not how the questions are delivered, but whose head the answers are in. Answering them yourself faster is `matt-grill-me`; getting them out of someone else is this.
...
- The answers that come back are usable input for a new matt-grilling round, rather than a fresh set of questions.

## Where it fits

`matt-to-questionnaire` is a reach-for-it-anytime standalone. It sits at the boundary of your own knowledge, where the next move is another person rather than another skill, most often mid-flow, when planning has stalled on something that isn't yours to decide.

Its neighbour is [matt-grill-me](../../skills/productivity/matt-grill-me/SKILL.md), and the two split on where the answers live: matt-grilling mines you, a questionnaire mines someone else. What comes back is raw material: feed it into another matt-grilling round, or into [matt-grill-with-docs](../../skills/engineering/matt-grill-with-docs/SKILL.md) or [matt-to-spec](../../skills/engineering/matt-to-spec/SKILL.md) if the work is heading for a build. When you're unsure which skill fits the moment, [matt-ask-matt](../../skills/engineering/matt-ask/SKILL.md) routes you.
